...
Page({
    ...
    wx.canvasToTempFilePath({
      ...
      success(res) {
        ...
        let ctx = that.data.ctx,
          ...
          signCanvasH = canvasWidth * canvasWidth / canvasHeight;
        ctx.translate(0, canvasWidth / 2.4);
        ctx.rotate(270 * Math.PI / 180);
        ctx.drawImage(res.tempFilePath, 0, 0, signCanvasH - 20, canvasWidth);
        ctx.draw(false, setTimeout(function () {
          if (that.data.type == 'sendTP' || that.data.type == 'joinStorageTP') {
            wx.canvasToTempFilePath({
              canvasId: 'handWriting',
              destWidth: 150,
              destHeight: 300,
              success: result => {
                ...
                let pages = getCurrentPages(), prePage = pages[0];
                let info = {
                  signSrc: result.tempFilePath,
                  role: that.data.role
                }
                prePage.setData({ info })
                wx.navigateBack({
                  delta: 1
                })
                wx.hideLoading()
          ...
          if(that.data.type == 'enterStorageTP'){
            wx.canvasToTempFilePath({
              canvasId: 'handWriting',
              destWidth: 50,
              destHeight: 100,
  ...
  onLoad: function (options) {
    ...
    console.log(options)
    this.setData({ role: options.role, type: options.type })
