...
Page({
  ...
  //签名
  signFn(e) {
    let role = e.target.dataset.role,type = 'sendTP';
    wx.navigateTo({
  ...
  onShow: function () {
    ...
    if (currentPage.data.info.signSrc) {
      ...
      Promise.all([
        ...
        //绘制签名(判断是收货单位还是送货单位签名)
        if (currentPage.data.info.role == 'sendUnit') {
          canvas.drawImage(res[1].path, 130, 620);
        }
        if (currentPage.data.info.role == 'receiveUnit') {
          canvas.drawImage(res[1].path, 590, 615);
        }
