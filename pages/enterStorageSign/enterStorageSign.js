...
Page({
  ...
  //签名
  signFn(e) {
    let role = e.target.dataset.role,type = 'enterStorageTP';
    wx.navigateTo({
  ...
  onShow: function () {
    ...
    if (currentPage.data.info.signSrc) {
      ...
      Promise.all([
        ...
        canvas.drawImage(res[0].path, 0, 0, 960, 708);

        //绘制签名
        if (currentPage.data.info.role == 'master') {
          console.log(res[1].path)
          canvas.drawImage(res[1].path, 82, 635);
        }
        if (currentPage.data.info.role == 'accountant') {
          canvas.drawImage(res[1].path, 212, 635);
        }
        if (currentPage.data.info.role == 'chargeUp') {
          canvas.drawImage(res[1].path, 342, 635);
        }
        if (currentPage.data.info.role == 'keep') {
          canvas.drawImage(res[1].path, 470, 635);
        }
        if (currentPage.data.info.role == 'acceptance') {
          canvas.drawImage(res[1].path, 607, 635);
        }
        if (currentPage.data.info.role == 'bestowal') {
          canvas.drawImage(res[1].path, 737, 635);
        }
        if (currentPage.data.info.role == 'make') {
          canvas.drawImage(res[1].path, 862, 635);
        }
