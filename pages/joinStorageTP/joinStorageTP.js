...
Page({
  ...
  uploadToCenter() {
    ...
    let uid = wx.getStorageSync('uid'), type = 0;
    wx.uploadFile({
  ...
  onShareAppMessage() {
    ...
    let url = encodeURIComponent('/pages/joinStorageSign/joinStorageSign?uploadedImgUrl=' + this.data.uploadedImgUrl + '&imgId=' + this.data.img_id);
    return {
      title: '签名',
  ...
  createOrder() {
    ...
    let promise1 = new Promise(function (resolve, reject) {
      wx.getImageInfo({
        src: '../../resources/templatesImgs/joinStorage.png',
    ...
    Promise.all([
      ...
      canvas.fillText(headData.NO, 760, 31)

      //绘制单位（公司名称）
      canvas.setFontSize(17)
      canvas.setFillStyle('black')
      canvas.fillText(headData.companyName, 80, 82)

      //绘制日期
      canvas.setFontSize(17)
      canvas.setFillStyle('black')
      canvas.fillText(headData.dateTime, 380, 82)

      //绘制类别
      canvas.setFontSize(17)
      canvas.setFillStyle('black')
      canvas.fillText(headData.type, 635, 82)

      //绘制编号
      canvas.setFontSize(17)
      canvas.setFillStyle('black')
      canvas.fillText(headData.No, 800, 82)
      ...
      for (let index = 0; index < rowsData.length; index++) {
        ...
        canvas.setFontSize(17)
        canvas.setFillStyle('black')
        canvas.fillText(rowsData[index].goodNo, 20, rowY)
        ...
        canvas.setFontSize(17)
        canvas.setFillStyle('black')
        canvas.fillText(rowsData[index].size, 345, rowY)

        //绘制单位
        canvas.setFontSize(17)
        canvas.setFillStyle('black')
        canvas.fillText(rowsData[index].unit, 505, rowY)

        //绘制数量
        canvas.setFontSize(17)
        canvas.setFillStyle('black')
        canvas.fillText(rowsData[index].goodCount, 565, rowY)

        //绘制单价
        canvas.setFontSize(17)
        canvas.setFillStyle('black')
        canvas.fillText(rowsData[index].goodMoney, 660, rowY)

        //绘制金额
        canvas.setFontSize(17)
        canvas.setFillStyle('black')
        canvas.fillText(rowsData[index].goodTotal, 760, rowY)

        //绘制备注
        canvas.setFontSize(17)
        canvas.setFillStyle('black')
        canvas.fillText(rowsData[index].msg, 870, rowY)
      ...
      canvas.setFontSize(17)
      canvas.setFillStyle('black')
      canvas.fillText(allMoneyTotalStr, 120, 595)

      //绘制全部总额(合计大写右边￥)
      canvas.setFontSize(17)
      canvas.setFillStyle('black')
      canvas.fillText(allMoneyTotal, 760, 588)

      //绘制仓库
      canvas.setFontSize(17)
      canvas.setFillStyle('black')
      canvas.fillText(footData.storage, 410, 635)
