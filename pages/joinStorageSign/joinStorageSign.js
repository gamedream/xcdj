...
Page({
  ...
  //签名
  signFn(e) {
    let role = e.target.dataset.role, type = 'joinStorageTP';
    wx.navigateTo({
  ...
  onShow: function () {
    ...
    console.log(that.data.orderSrcNet)
    wx.getImageInfo({
    ...
    if (currentPage.data.info.signSrc) {
      ...
      Promise.all([
        ...
        canvas.drawImage(res[0].path, 0, 0, 967, 677);

        //入库单--绘制签名(判断是主管签名还是经手人签名)
        if (currentPage.data.info.role == 'master') {
          canvas.drawImage(res[1].path, 90, 610);
        }
        if (currentPage.data.info.role == 'handler') {
          canvas.drawImage(res[1].path, 790, 610);
        }
